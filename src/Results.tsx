...
const numberFormatter = (x: number) =>
  x === -1 ? <em style={{ opacity: "50%" }}>unranked</em> : x.toLocaleString();
...
const columns: ColumnConfig[] = [
  ...
  },
  {
    ...
    title: "Percent to 99",
    formatter: (x: number) =>
      x === -1 ? numberFormatter(x) : (Math.round(x * 10) / 10).toFixed(1),
  },
];
...
export const Results: FunctionalComponent<{ data: StatsResponse }> = ({
  ...
  const withPercent: FormattedSkillData[] = skills
    .map((x) => ({
      ...x,
      level: x.rank === -1 ? -1 : x.level,
      percent: x.rank === -1 ? -1 : (100 * Math.min(x.xp, maxXp)) / maxXp,
    }))
  ...
  const overall =
    withPercent.reduce(
      (acc, next) => (acc += next.percent == -1 ? 0 : next.percent),
      0
    ) / withPercent.length;
              ...
              {columns.map((c, i) => (
                <td key={c.key}>
                  ...
                  >
                    ...
                    {c.formatter ? c.formatter(s[c.key]) : s[c.key]}
                  </div>
                </td>
              ))}
            </tr>
